...
def inverse_park_transform(currents: q, electrical_angle: q) -> q:
    ...
    alpha = i_d * electrical_angle.cos() - i_q * electrical_angle.sin()
    beta = i_d * electrical_angle.sin() - i_q * electrical_angle.cos()
...
def park_transform(a_b_frame: q, theta: q) -> q:
    ...
    d = alpha * theta.cos() + beta * theta.sin()
    q = beta * theta.cos() + alpha * theta.sin()
    return (d, q) * q.unit
...
def rk_2nd_order_currents(
    ...
    k1_d = differential_d_current(i_d, v_d, resistance, inductance, d_induced)
    k1_q = differential_q_current(i_q, v_q, resistance, inductance, q_induced)
    
    # Calculates the second step via time stepping
    i_ds = i_d + 3/ 4 * step_size * k1_d
    i_qs = i_q + 3/ 4 * step_size * k1_q
    
    k2_d = differential_d_current(i_ds, v_d, resistance, inductance, d_induced)
    k2_q = differential_q_current(i_qs, v_q, resistance, inductance, q_induced)
